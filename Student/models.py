...
class Ticket(models.Model):
    ...
    def escalate(self, escalated_by, reason=None):
        ...
        # Check if ticket is already properly escalated (has all escalation data)
        if self.status == 'escalated' and self.original_department and self.escalated_at and self.escalated_by:
            print(f"Ticket {self.ticket_id} is already properly escalated")
            return
        
        # If status is escalated but missing escalation data, complete the escalation process
        if self.status == 'escalated' and not self.original_department:
            print(f"Ticket {self.ticket_id} has escalated status but missing escalation data. Completing escalation process.")
        
        # Store the original department before changing it (if not already stored)
