...
from django.db.models import Q, Count, ExpressionWrapper, F, Avg
...
from .forms import CreateStudentForm, UpdateComplaintForm
from .utils import process_excel_file, send_registration_sms, process_student_registrations, complete_student_registration
from .decorators import dept_admin_required
from core.models import Department, Profile
from Student.models import Ticket, SLABreachLog, TicketUpdate
from core.utils import calculate_sla_due
...
def login_view(request):
    ...
    # get_token(request) # This line was removed from the new_code, so it's removed here.
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user is not None:
                ...
                # rotate_token(request) # This line was removed from the new_code, so it's removed here.
    ...
    # response.set_cookie( # This block was removed from the new_code, so it's removed here.
                ...
                try:
                    ...
                    if user:
                        ...
                        if hasattr(user, 'profile'):
                            ...
                            profile.save()
                            print("[DEBUG] Profile ensured for new user. Sending SMS and email.")
                            ...
                            <body>
                                <div class='container'>
                                    ...
                                    <div class='content'>
                                        <p>Hello {form.cleaned_data['first_name']},</p>
                            ...
                            if mail_sent:
                                messages.success(request, f'Student account created successfully! Roll Number: {roll_number}, Email: {email} (Email sent)')
                            else:
                                messages.warning(request, f'Student account created successfully! Roll Number: {roll_number}, Email: {email} (Email failed to send)')
                        form = CreateStudentForm()
                        return render(request, 'dept_admin/create_student.html', {'form': form})
                    else:
